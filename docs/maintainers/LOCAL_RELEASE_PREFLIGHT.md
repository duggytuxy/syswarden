...
| `qualification-continuity.yml` | Exact PR257 policy, original raw-proof revalidation, seven retained and four fresh performance metrics, source allocations, portable private-input digests and distinct runtime/tooling provenance | Owner-approved qualification environment, dedicated ephemeral AMD64 runner, exact reviewed input manifest, tooling-bound GitHub attestation and performance-only artifact; all other release gates remain mandatory |
