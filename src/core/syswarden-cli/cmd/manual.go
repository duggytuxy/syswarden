...
var manualCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("  %slist%s                : Displays the manual IP registries.\n", ansiGreen, ansiReset)
		...
		fmt.Printf("  %supdate%s              : Runs the legacy in-place package updater; see the warning below.\n", ansiGreen, ansiReset)
		...
		fmt.Printf("  Native TUI trust: /etc/syswarden/ha-ca.pem when present, otherwise system roots. Restart the TUI after trust changes.\n")
		fmt.Printf("  Transfer server.crt through a trusted out-of-band channel and verify its fingerprint before trusting it.\n")
		fmt.Printf("  The separate syswarden ha-sync client still disables certificate verification; tokenless legacy mode exists.\n")
		fmt.Printf("  Automatic HA synchronization runs the same one-way push every 30 minutes; it does not pull or remove peer-only entries.\n\n")

		fmt.Printf("%s--- 5. SAFETY LIMITS ---%s\n", ansiYellow, ansiReset)
		fmt.Printf("  %sRELOAD:%s firewall rules are reapplied and the core normally restarts; keep console access and a ruleset backup.\n", ansiRed, ansiReset)
		fmt.Printf("  %sUPDATE:%s downloaded packages are not currently verified by checksum or signature; do not use the legacy updater.\n", ansiRed, ansiReset)
