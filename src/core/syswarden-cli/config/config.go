...
type Config struct {
	...
	HAPeerID                   string
	HARole                     string
	HAV2SecretFile             string
	HATLSCertFile              string
	HATLSKeyFile               string
	HATLSCAFile                string
	HAPeerTLSName              string
	HAPeerCertSHA256           []string
	HAStateFile                string
	HATransactionFile          string
	HAHeartbeatIntervalSeconds int
	HAHeartbeatTimeoutSeconds  int
	HARequestTimeoutSeconds    int
	SiemEnabled                bool
	SiemIP                     string
	SiemPort                   string
	SiemProto                  string
...
func NewFailSafeConfig() *Config {
	return &Config{
		...
		WGPort:                     "51820",
		HAPeerPort:                 "62026",
		HAStateFile:                "/var/lib/syswarden/ha/replication-v2.json",
		HATransactionFile:          "/var/lib/syswarden/ha/replication-v2.wal.json",
		HAHeartbeatIntervalSeconds: 2,
		HAHeartbeatTimeoutSeconds:  10,
		HARequestTimeoutSeconds:    5,
		SiemPort:                   "6514",
		SiemProto:                  "tls",
	}
}
