...
const validTransportOperatorPolicyModule = `[[operator_policy.rules]]
id = "allow-tcp-v4"
family = "ipv4"
direction = "ingress"
protocol = "tcp"
...
func TestOperatorPolicyTCPUDPMapsWithoutWeakCoercion_SW_OPPOL_013(t *testing.T) {
	preserveOperatorPolicyGlobals(t)
	root := operatorPolicyFixture(t, validTransportOperatorPolicyModule)
	if err := loadModularConfig(root); err != nil {
		t.Fatalf("load typed transport policy: %v", err)
	}
	if len(GlobalConfig.OperatorPolicy.Rules) != 2 ||
		GlobalConfig.OperatorPolicy.Rules[0].DestinationPort != 8443 ||
		GlobalConfig.OperatorPolicy.Rules[1].DestinationPort != 51820 {
		t.Fatalf("typed transport policy mismatch: %#v", GlobalConfig.OperatorPolicy)
	}
	for _, invalid := range []string{
		strings.Replace(validTransportOperatorPolicyModule, "destination_port = 8443", `destination_port = "8443"`, 1),
		strings.Replace(validTransportOperatorPolicyModule, "destination_port = 8443", "destination_port = 0", 1),
		strings.Replace(validTransportOperatorPolicyModule, "destination_port = 8443", "destination_port = 65536", 1),
		strings.Replace(validTransportOperatorPolicyModule, "destination_port = 8443", "destination_port = 8443\nraw = \"tcp dport 22 accept\"", 1),
	} {
		if _, err := parseTOMLDocument([]byte(invalid), operatorPolicyModulePath); err == nil {
			t.Fatalf("invalid transport policy was accepted:\n%s", invalid)
		}
	}
}
