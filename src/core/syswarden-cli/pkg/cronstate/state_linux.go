...
func validateOptions(options Options) error {
	...
	if options.DirectoryOwnerUID < 0 || options.DirectoryOwnerGID < 0 || options.FileOwnerUID < 0 || options.FileOwnerGID < 0 {
		return fmt.Errorf("cron ownership identifiers must be non-negative")
	}
...
func inspectFile(directory *os.Root, name string, uid, gid int, allowedModes ...fs.FileMode) (fileIdentity, []byte, bool, error) {
	...
	if stat.Uid != uint32(uid) || stat.Gid != uint32(gid) || stat.Nlink != 1 {
...
func validateTransactionRecord(record transactionRecord, options Options) error {
	...
	validateIdentity := func(label string, identity journalIdentity) error {
		...
		if identity.Device == 0 || identity.Inode == 0 || identity.Mode != 0644 ||
			...
			identity.UID != uint32(options.FileOwnerUID) || identity.GID != uint32(options.FileOwnerGID) ||
			identity.Links != 1 {
...
func createNamed(
	...
	if stat.Uid != uint32(options.FileOwnerUID) || stat.Gid != uint32(options.FileOwnerGID) {
