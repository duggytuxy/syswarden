...
func testCronOptions(t *testing.T, reader *testLegacyReader) (Options, string) {
	t.Helper()
	rootPath := t.TempDir()
	if err := os.Chmod(rootPath, 0700); err != nil { // #nosec G302 -- owner-only mode is required for the private test root
		t.Fatal(err)
	}
	...
	// #nosec G703 -- rootPath is the exact private test root supplied by this package
...
func testCronOptionsAt(t *testing.T, rootPath string, reader *testLegacyReader) (Options, string) {
	t.Helper()
	cronPath := filepath.Join(rootPath, "etc", "cron.d")
	// #nosec G703 -- rootPath is an exact private fixture root supplied by its test caller
	rootInfo, err := os.Stat(rootPath)
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidateOptionsRejectsOwnershipOutsideUint32_SW2_CRON_001(t *testing.T) {
	reader := &testLegacyReader{}
	fields := []struct {
		name string
		set  func(*Options, int)
	}{
		{name: "directory uid", set: func(options *Options, value int) { options.DirectoryOwnerUID = value }},
		{name: "directory gid", set: func(options *Options, value int) { options.DirectoryOwnerGID = value }},
		{name: "file uid", set: func(options *Options, value int) { options.FileOwnerUID = value }},
		{name: "file gid", set: func(options *Options, value int) { options.FileOwnerGID = value }},
	}
	for _, field := range fields {
		t.Run(field.name+" negative", func(t *testing.T) {
			options := DefaultOptions(reader.read)
			field.set(&options, -1)
			if err := validateOptions(options); err == nil {
				t.Fatal("negative ownership identifier was accepted")
			}
		})
		if strconv.IntSize == 64 {
			t.Run(field.name+" above uint32", func(t *testing.T) {
				options := DefaultOptions(reader.read)
				aboveUint32 := int(int64(^uint32(0)) + 1)
				field.set(&options, aboveUint32)
				if err := validateOptions(options); err == nil {
					t.Fatal("ownership identifier above uint32 was accepted")
				}
			})
		}
	}
...
func TestFreshFeedPublicationIsCanonicalAndIdempotent(t *testing.T) {
	...
	if !ok || int64(ownedStat.Uid) != int64(options.FileOwnerUID) || int64(ownedStat.Gid) != int64(options.FileOwnerGID) {
...
func TestUnsafeOwnedFileIdentitiesFailClosed(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			create: func(path string) error {
				return os.WriteFile(path, []byte(ownedCronHeader+ownedHALine(platformpaths.CLI)+"\n"), 0644) // #nosec G306 -- fixture deliberately models the deployed 0644 cron contract
			},
...
func TestSymlinkedOrSwappedCronParentsFailClosed(t *testing.T) {
	t.Run("symlinked cron directory", func(t *testing.T) {
		...
		if err := os.Mkdir(operatorDirectory, 0755); err != nil { // #nosec G301 -- fixture deliberately models a normal system directory mode
			t.Fatal(err)
		}
...
func TestDestinationSwapBeforeReplaceIsDetected(t *testing.T) {
	...
	options.beforeReplace = func() {
		if err := os.WriteFile(ownedPath(cronPath), []byte("operator drift\n"), 0644); err != nil { // #nosec G306 -- fixture deliberately replaces the deployed 0644 cron file
			t.Fatal(err)
		}
	}
	if err := ReconcileHA(options, true); err == nil {
...
func TestRemoveForUninstallPreflightsAllStateBeforeMutation(t *testing.T) {
	...
	t.Run("malformed owned file preserves safe pending residue", func(t *testing.T) {
		reader := &testLegacyReader{}
		options, cronPath := testCronOptions(t, reader)
		if err := os.WriteFile(ownedPath(cronPath), []byte("operator owned\n"), 0644); err != nil { // #nosec G306 -- fixture deliberately models an operator-owned 0644 cron file
			t.Fatal(err)
		}
		...
		for path, want := range map[string]string{
			...
			content, err := os.ReadFile(path) // #nosec G304 -- path is selected from fixed names beneath the private test root
...
func runCronSIGKILLHelper(t *testing.T, rootPath, action, fault string) {
	t.Helper()
	command := exec.Command(os.Args[0], "-test.run=^TestCronTransactionSIGKILLHelper$") // #nosec G204 G702 -- executable is the current test binary and arguments are fixed
...
func TestDurableCronTransactionRecoversRealSIGKILLBoundaries(t *testing.T) {
	t.Run("creation rollback and retry", func(t *testing.T) {
		...
		journalWire, err := os.ReadFile(filepath.Join(cronPath, transactionFileName)) // #nosec G304 -- fixed journal name is beneath the private test root
		if err != nil {
			t.Fatal(err)
		}
		...
		t.Run(drift, func(t *testing.T) {
			...
			snapshot := func() map[string]string {
				...
				for _, entry := range entries {
					content, err := os.ReadFile(filepath.Join(cronPath, entry.Name())) // #nosec G304 -- entry came from ReadDir on the private cron fixture
					if err != nil {
						t.Fatal(err)
					}
...
func TestCronPinnedRemovalNeverTouchesSwappedCanonicalDirectory(t *testing.T) {
	...
	options.unlinkat = func(directory int, name string, flags int) error {
		if !swapped {
			...
			if err := os.Mkdir(cronPath, 0755); err != nil { // #nosec G301 -- fixture deliberately restores the system cron directory mode
				t.Fatal(err)
			}
			if err := os.WriteFile(ownedPath(cronPath), operatorContent, 0644); err != nil { // #nosec G306 -- fixture deliberately models an operator-owned 0644 cron file
				t.Fatal(err)
			}
		}
		return originalUnlink(directory, name, flags)
...
func TestCronCommittedCleanupPreservesExternalQuarantineCollision(t *testing.T) {
	...
	options.renameat2 = func(oldDirectory int, oldName string, newDirectory int, newName string, flags uint) error {
		if !injected && oldName == pendingFileName && newName == retiredFileName && flags == unix.RENAME_NOREPLACE {
			injected = true
			if err := os.WriteFile(filepath.Join(cronPath, retiredFileName), []byte("operator retirement\n"), 0644); err != nil { // #nosec G306 -- fixture deliberately models an operator-owned 0644 retirement file
				t.Fatal(err)
			}
		}
		return originalRename(oldDirectory, oldName, newDirectory, newName, flags)
	...
	retired, err := os.ReadFile(filepath.Join(cronPath, retiredFileName)) // #nosec G304 -- fixed retirement name is beneath the private test root
