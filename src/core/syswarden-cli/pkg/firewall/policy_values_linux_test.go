...
func TestManagedLinuxWrapperDiscoveryRequiresAnActiveBackend_SW_FW_001(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "inactive firewalld after dbus connection wait",
			executable:  "firewall-cmd",
			backend:     "firewalld",
			script:      "#!/bin/sh\nprintf 'Waiting on dbus connection...\nnot running\n' >&2\nexit 252\n",
			wantDormant: true,
		},
		{
			name:       "active firewalld after dbus connection wait",
			executable: "firewall-cmd",
			backend:    "firewalld",
			script:     "#!/bin/sh\nprintf 'Waiting on dbus connection...\n' >&2\nprintf 'running\n'\n",
			wantActive: true,
		...
		{
			name:       "firewalld connection wait without state",
			executable: "firewall-cmd",
			backend:    "firewalld",
			script:     "#!/bin/sh\nprintf 'Waiting on dbus connection...\n' >&2\nexit 252\n",
		...
		{
			name:       "firewalld connection wait with conflicting states",
			executable: "firewall-cmd",
			backend:    "firewalld",
			script:     "#!/bin/sh\nprintf 'Waiting on dbus connection...\nrunning\nnot running\n'\nexit 252\n",
			wantError:  "query firewalld service state",
		...
		{
			name:       "firewalld connection wait with unexpected exit status",
			executable: "firewall-cmd",
			backend:    "firewalld",
			script:     "#!/bin/sh\nprintf 'Waiting on dbus connection...\nnot running\n' >&2\nexit 7\n",
			wantError:  "query firewalld service state",
		...
		{
			name:       "firewalld unknown diagnostic before state",
			executable: "firewall-cmd",
			backend:    "firewalld",
			script:     "#!/bin/sh\nprintf 'dbus permission denied\nnot running\n' >&2\nexit 252\n",
			wantError:  "query firewalld service state",
		},
		{
