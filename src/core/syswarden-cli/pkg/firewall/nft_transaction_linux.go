...
func newExecNFTCommandRunner() (nftCommandRunner, error) {
	...
	return execNFTCommandRunner{path: resolved}, nil
}
...
func (runner execNFTCommandRunner) Run(ctx context.Context, stdin []byte, args ...string) ([]byte, error) {
	...
	case len(args) == 3 && args[0] == "-j" && args[1] == "list" && args[2] == "tables":
		cmd = exec.CommandContext(ctx, runner.path, "-j", "list", "tables")
	case len(args) == 3 && args[0] == "-j" && args[1] == "list" && args[2] == "ruleset":
		cmd = exec.CommandContext(ctx, runner.path, "-j", "list", "ruleset")
	case len(args) == 4 && args[0] == "list" && args[1] == "table":
		...
		case nftTableTarget{family: "inet", name: "syswarden"}:
			cmd = exec.CommandContext(ctx, runner.path, "list", "table", "inet", "syswarden")
		case nftTableTarget{family: "inet", name: "syswarden_table"}:
			cmd = exec.CommandContext(ctx, runner.path, "list", "table", "inet", "syswarden_table")
		case nftTableTarget{family: "netdev", name: "syswarden_hw_drop"}:
			cmd = exec.CommandContext(ctx, runner.path, "list", "table", "netdev", "syswarden_hw_drop")
		case nftTableTarget{family: "arp", name: "syswarden_arp"}:
			cmd = exec.CommandContext(ctx, runner.path, "list", "table", "arp", "syswarden_arp")
		default:
	...
	case len(args) == 4 && args[0] == "delete" && args[1] == "table":
		...
		cmd = exec.CommandContext(ctx, runner.path, "delete", "table", target.family, target.name)
	case len(args) == 3 && args[0] == "-c" && args[1] == "-f":
		...
		cmd = exec.CommandContext(ctx, runner.path, "-c", "-f", "/proc/self/fd/3")
	...
	case len(args) == 2 && args[0] == "-f":
		...
		cmd = exec.CommandContext(ctx, runner.path, "-f", "/proc/self/fd/3")
		cmd.ExtraFiles = []*os.File{commandFile}
	default:
