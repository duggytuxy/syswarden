...
func TestExternalCommandContractRejectsUnexpectedArguments(t *testing.T) {
	...
	invalid := []struct {
		...
		{name: "/usr/bin/systemctl", args: []string{"restart", "syswarden-webtui"}},
		{name: "/usr/sbin/rc-service", args: []string{"syswarden-webtui", "restart"}},
	}
...
func TestFinishUpgradeReturnsPartialAndMultipleActivationFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := u.finishUpgrade(t.Context(), packageTarget{os: "linux", format: packageFormatDEB})
			if err == nil {
			...
			if calls.Load() != 2 {
				t.Errorf("activation command calls = %d, want 2", calls.Load())
			}
...
func TestFinishUpgradeReportsRetiredConfigurationCleanupFailure(t *testing.T) {
	t.Parallel()
	...
	err := u.finishUpgrade(t.Context(), packageTarget{os: "linux", format: packageFormatDEB})
	if err == nil || !strings.Contains(err.Error(), "retire Web-TUI configuration") ||
		!strings.Contains(err.Error(), "injected cleanup failure") {
		t.Fatalf("retired configuration cleanup error = %v", err)
	}
	if strings.Contains(output.String(), "completed successfully") {
		t.Fatalf("failed cleanup printed a success message: %q", output.String())
	}
}
