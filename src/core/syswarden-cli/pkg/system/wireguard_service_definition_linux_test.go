...
func openRCWireGuardFixture(t *testing.T) (openRCWireGuardDefinitionPaths, string, string, string) {
	...
	for _, directory := range []string{"lib/apk/db", "etc/init.d"} {
		if err := os.MkdirAll(filepath.Join(root, directory), 0755); err != nil { // #nosec G301 -- fixture models OpenRC and APK system directory modes under a private root
			t.Fatal(err)
		}
	}
	scriptPath := filepath.Join(root, "etc/init.d/wg-quick")
	script := []byte("#!/sbin/openrc-run\ncommand=/usr/bin/wg-quick\n")
	if err := os.WriteFile(scriptPath, script, 0755); err != nil { // #nosec G306 -- executable mode is required for the isolated OpenRC fixture
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(databasePath, []byte(database), 0644); err != nil { // #nosec G306 -- fixture models the read-only APK database mode under a private root
		t.Fatal(err)
	}
	...
	return openRCWireGuardDefinitionPaths{
		root: root, expectedUID: systemTestUID(t), expectedGID: systemTestGID(t),
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(t *testing.T, _, script, _ string) {
				if err := os.WriteFile(script, []byte("#!/bin/sh\nexit 0\n"), 0755); err != nil { // #nosec G306 -- executable mode is required for the adversarial OpenRC fixture
					t.Fatal(err)
				}
			},
		},
		{
			...
			mutate: func(t *testing.T, _, script, _ string) {
				if err := os.Chmod(script, 0775); err != nil { // #nosec G302 -- adversarial fixture deliberately proves group-writable scripts are rejected
					t.Fatal(err)
				}
			},
		},
		{
			...
			mutate: func(t *testing.T, database, _, _ string) {
				content, err := os.ReadFile(database) // #nosec G304 -- database is the exact private fixture path supplied by the table
				if err != nil {
					t.Fatal(err)
				}
				// #nosec G306 G703 -- database is the exact private fixture path and mode models the APK database
		...
		},
		{
			...
			mutate: func(t *testing.T, database, _, _ string) {
				if err := os.Chmod(database, 0664); err != nil { // #nosec G302 -- adversarial fixture deliberately proves writable package metadata is rejected
					t.Fatal(err)
				}
			},
		},
