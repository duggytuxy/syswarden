...
var (
	...
	errHistoricalDefaultSystemdFirewallCompatibilityEligible = errors.New(
		"historical default systemd firewall compatibility is eligible",
	)
	errHistoricalDefaultOpenRCFirewallCompatibilityEligible = errors.New(
		"historical default OpenRC firewall compatibility is eligible",
	)
)

// IsHistoricalDefaultSystemdFirewallCompatibilityEligible reports whether the
// explicit nftables preflight proved the exact dormant systemd service state.
func IsHistoricalDefaultSystemdFirewallCompatibilityEligible(err error) bool {
	return errors.Is(err, errHistoricalDefaultSystemdFirewallCompatibilityEligible)
}

// IsHistoricalDefaultOpenRCFirewallCompatibilityEligible reports whether the
// explicit nftables preflight proved the exact dormant OpenRC service state.
func IsHistoricalDefaultOpenRCFirewallCompatibilityEligible(err error) bool {
	return errors.Is(err, errHistoricalDefaultOpenRCFirewallCompatibilityEligible)
}
...
func isHistoricalDefaultSystemdFirewallCompatibilitySnapshot(snapshot [6]firewallUnitSnapshot) bool {
	wantNames := [6]string{
		"nftables.service",
		"firewalld.service",
		"ufw.service",
	...
	for index, unit := range snapshot {
		if unit.name != wantNames[index] {
			return false
		}
	}
	target := snapshot[0]
	if !target.loaded || target.enabled || target.active {
		return false
	}
	for _, frontend := range snapshot[1:] {
...
func inspectHistoricalDefaultOpenRCFirewallSnapshot(
	executor firewallManagerExecutor,
	servicePath string,
	runlevelPath string,
) ([6]openRCFirewallUnitSnapshot, error) {
	names := [6]string{"nftables", "firewalld", "ufw", "iptables", "ip6tables", "netfilter-persistent"}
	...
	if err != nil {
		return [6]openRCFirewallUnitSnapshot{}, fmt.Errorf("inspect historical default OpenRC firewall runlevels: %w", err)
	}
	enabled, err := inspectOpenRCRunlevelMembership(runlevelOutput, tracked)
	if err != nil {
		return [6]openRCFirewallUnitSnapshot{}, err
	}
	var snapshot [6]openRCFirewallUnitSnapshot
	for index, name := range names {
		...
		if existsErr != nil {
			...
			if !exact || code != 1 {
				return [6]openRCFirewallUnitSnapshot{}, fmt.Errorf("inspect historical default OpenRC firewall service %s: %w", name, existsErr)
			}
			if state.enabled {
				return [6]openRCFirewallUnitSnapshot{}, fmt.Errorf("absent historical default OpenRC firewall service %s remains enabled", name)
			}
		...
		if statusErr == nil {
			...
			if !exact || code != 3 {
				return [6]openRCFirewallUnitSnapshot{}, fmt.Errorf("inspect historical default OpenRC firewall service status for %s: %w", name, statusErr)
			}
		}
		snapshot[index] = state
	}
...
func validateHistoricalDefaultOpenRCFirewallSnapshot(snapshot [6]openRCFirewallUnitSnapshot) error {
	wantNames := [6]string{"nftables", "firewalld", "ufw", "iptables", "ip6tables", "netfilter-persistent"}
	for index, unit := range snapshot {
		if unit.name != wantNames[index] {
			return fmt.Errorf("historical default OpenRC firewall inventory changed at index %d", index)
		}
	}
	target := snapshot[0]
	if !target.loaded {
		return fmt.Errorf("required historical default OpenRC firewall service nftables is absent")
	}
	if target.active || target.enabled {
		return fmt.Errorf("required historical default OpenRC firewall service nftables must be stopped and disabled")
	}
	for _, conflict := range snapshot[1:] {
		if conflict.loaded || conflict.active || conflict.enabled {
			return fmt.Errorf("historical default OpenRC firewall conflict %s must be absent, inactive, and disabled", conflict.name)
		}
	}
	return nil
}

func preflightHistoricalDefaultOpenRCFirewallCompatibility(executor firewallManagerExecutor) error {
	...
	initial, err := inspectHistoricalDefaultOpenRCFirewallSnapshot(executor, servicePath, runlevelPath)
	if err != nil {
		return err
	}
	if err := validateHistoricalDefaultOpenRCFirewallSnapshot(initial); err != nil {
		return err
	}
	confirmed, err := inspectHistoricalDefaultOpenRCFirewallSnapshot(executor, servicePath, runlevelPath)
	if err != nil {
		return fmt.Errorf("reinspect historical default OpenRC firewall state: %w", err)
	}
	if confirmed != initial {
		return fmt.Errorf("historical default OpenRC firewall state changed during preflight")
	}
	if err := validateHistoricalDefaultOpenRCFirewallSnapshot(confirmed); err != nil {
		return fmt.Errorf("revalidate historical default OpenRC firewall state: %w", err)
	}
	return fmt.Errorf(
		"%w: nftables is loaded, stopped, and disabled while all conflicting services are absent",
		errHistoricalDefaultOpenRCFirewallCompatibilityEligible,
	)
}
...
func preflightHostFirewallBackend(
	...
	if alpine {
		return preflightHistoricalDefaultOpenRCFirewallCompatibility(executor)
	}
	...
	initialValidationErr := validateNftablesSnapshot(initial)
	compatibilityEligible := isHistoricalDefaultSystemdFirewallCompatibilitySnapshot(initial)
	if initialValidationErr != nil && !compatibilityEligible {
		return initialValidationErr
	}
	...
	if compatibilityEligible {
		if !isHistoricalDefaultSystemdFirewallCompatibilitySnapshot(confirmed) {
			return fmt.Errorf("revalidate historical default firewall compatibility state")
		}
		return fmt.Errorf(
			"%w: nftables.service is loaded, disabled, and inactive with no active or enabled conflicting frontend",
			errHistoricalDefaultSystemdFirewallCompatibilityEligible,
		)
	}
