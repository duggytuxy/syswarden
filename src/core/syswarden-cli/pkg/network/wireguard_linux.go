...
var wireGuardMasqueradeInterface = regexp.MustCompile(`oifname "([A-Za-z0-9_.:-]{1,15})" masquerade`)
...
func wireGuardRootedPath(logical string) (string, error) {
	if wireGuardFilesystemRoot == "" || !filepath.IsAbs(wireGuardFilesystemRoot) ||
		filepath.Clean(wireGuardFilesystemRoot) != wireGuardFilesystemRoot {
		return "", fmt.Errorf("WireGuard filesystem root must be canonical and absolute")
	}
	if !filepath.IsAbs(logical) || filepath.Clean(logical) != logical || logical == "/" {
		return "", fmt.Errorf("WireGuard path must be canonical and absolute")
	}
	if wireGuardFilesystemRoot == "/" {
		return logical, nil
	}
	return filepath.Join(wireGuardFilesystemRoot, strings.TrimPrefix(logical, "/")), nil
}
...
func wireGuardActiveInterfaceFromConfiguration(serverConfiguration string) (string, error) {
	matches := wireGuardMasqueradeInterface.FindAllStringSubmatch(serverConfiguration, -1)
	if len(matches) != 1 || len(matches[0]) != 2 || !wireGuardInterfaceName.MatchString(matches[0][1]) {
		return "", fmt.Errorf("WireGuard server configuration has no unique validated masquerade interface")
	}
