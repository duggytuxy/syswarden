...
func TestWireGuardRetainedFDHelper(t *testing.T) {
	...
	child := exec.Command( // #nosec G204 -- current test executable and private absolute test marker
		executable,
...
func TestWireGuardCommandWaitDelayBoundsChildRetainedDescriptors_SW2_WGSTATE_001(t *testing.T) {
	...
	wireGuardExecutableIdentityCapture = func(path string) (wireGuardExecutableIdentity, error) {
		return captureWireGuardExecutableIdentityForOwner(path, uint32(os.Geteuid()), false)
	}
	t.Cleanup(func() {
	...
	for _, streaming := range []bool{false, true} {
		t.Run(fmt.Sprintf("streaming-%v", streaming), func(t *testing.T) {
			...
			wireGuardExecutablePreStartAttestor = func(identity wireGuardExecutableIdentity) error {
				...
				if err := os.WriteFile(executable, replacement, 0700); err != nil {
					t.Fatal(err)
				}
				swapped = true
			...
			var err error
			if streaming {
				stdout, openErr := os.OpenFile(filepath.Join(directory, "stdout"), os.O_CREATE|os.O_WRONLY, 0600)
				if openErr != nil {
					t.Fatal(openErr)
				}
