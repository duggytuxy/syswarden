...
func installWireGuardTransactionHarness(t *testing.T) *wireGuardTransactionHarness {
	...
	if err := os.Mkdir(filepath.Join(harness.root, "etc"), 0755); err != nil {
		t.Fatal(err)
	}
...
func TestSetupWireGuardPublishesAttestedTransactionBeforeMutation_SW2_WGSTATE_001(t *testing.T) {
	...
	manifest, err := wireguardstate.ReadAndVerify(
		harness.root, uint32(os.Geteuid()), uint32(os.Getegid()),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetupWireGuardReusesOnlyFullyAttestedState_SW2_WGSTATE_001(t *testing.T) {
	...
	before, err := wireguardstate.ReadAndVerify(harness.root, uint32(os.Geteuid()), uint32(os.Getegid()))
	if err != nil {
		t.Fatal(err)
	}
	...
	after, err := wireguardstate.ReadAndVerify(harness.root, uint32(os.Geteuid()), uint32(os.Getegid()))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetupWireGuardPostActivationDriftRollsBackServiceThenOwnedNFT_SW2_FWBACKEND_001(t *testing.T) {
	...
	if _, err := wireguardstate.ReadAndVerify(harness.root, uint32(os.Geteuid()), uint32(os.Getegid())); err != nil {
...
func TestSetupWireGuardRejectsPartialStateBeforeKeyOrHostMutation_SW2_WGSTATE_001(t *testing.T) {
	...
	if err := os.Chmod(filepath.Dir(serverPath), 0755); err != nil {
		t.Fatal(err)
	}
...
func TestPinnedWireGuardForwardingTransactionRestoresExactValueAndRejectsPathSwap_SW2_WGSTATE_001(t *testing.T) {
	t.Run("apply and restore", func(t *testing.T) {
		...
		transaction, err := openPinnedWireGuardForwardingTransaction(path, uint32(os.Geteuid()))
		if err != nil {
			t.Fatal(err)
		}
		...
		if wire, err := os.ReadFile(path); err != nil || strings.TrimSpace(string(wire)) != "1" {
			t.Fatalf("forwarding apply: content=%q err=%v", wire, err)
		}
		if err := transaction.Restore(); err != nil {
			t.Fatal(err)
		}
		if wire, err := os.ReadFile(path); err != nil || strings.TrimSpace(string(wire)) != "0" {
