...
if [[ -f "${1:-}" ]]; then
    ...
    while IFS= read -r line; do
        ...
        # Only allows safe characters in values, including Webhook specific characters (?, &, =, @, %) and wildcards (*).
...
if [[ "$MODE" != "update" ]] && [[ "$MODE" != "uninstall" ]]; then
    ...
    echo -e "${RED}╚══════╝   ╚═╝   ╚══════╝ ╚══╝╚══╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═════╝ ╚══════╝╚═╝  ╚═══╝${NC}"
    echo -e "${BLUE}===================================================================================${NC}"
    echo -e "${GREEN}               Host-based Security Orchestrator for Linux. | v0.42.10                  ${NC}"
    echo -e "${BLUE}===================================================================================${NC}\n"
fi
...
if [[ "$MODE" != "update" ]]; then
    ...
    if [[ "$MODE" != "auto" ]]; then
        ...
        clear
        echo -e "${BLUE}${BOLD}==============================================================================${NC}"
        echo -e "${GREEN}${BOLD}                   SYSWARDEN v0.42.10 - PRE-FLIGHT CHECKLIST                     ${NC}"
        echo -e "${BLUE}${BOLD}==============================================================================${NC}"
...
if [[ "$MODE" != "update" ]]; then
    ...
    # Apply advanced CIS Level 2 hardening (Defense-in-Depth)
