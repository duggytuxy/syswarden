...
failregex = (?i)^<HOST> \S+ \S+ (?:\[[^\]]*\]\s+)?"(?:GET|POST|HEAD|PUT) [^"]*?(?:(?:/|\x252f|\x25252f)(?:\.|\x252e|\x25252e)(?:env|git|aws|ssh)[^ ]*|(?:/|\x252f|\x25252f)id_(?:rsa|ed25519)[^ ]*|(?:/|\x252f|\x25252f)[^ ]*\.(?:sql|bak|swp|db|sqlite3?)(?:\.gz|\.zip)?|(?:/|\x252f|\x25252f)docker-compose\.ya?ml|(?:/|\x252f|\x25252f)wp-config\.php\.(?:bak|save|old|txt|zip)) HTTP/.*?" \d{3} .*$
ignoreregex = 
EOF
    fi
