...
failregex = ^<HOST> \S+ \S+ \[[^\]]*\] "(?:GET|POST|HEAD|PUT|PATCH|DELETE) [^"]*(?:UNION(?:\s|\+|\x2520)SELECT|CONCAT(?:\s|\+|\x2520)?\(|WAITFOR(?:\s|\+|\x2520)DELAY|SLEEP(?:\s|\+|\x2520)?\(|\x253Cscript|\x253E|\x253C\x252Fscript|<script|alert\(|onerror=|onload=|document\.cookie|base64_decode\(|eval\(|\.\./\.\./|\x252E\x252E\x252F|\.(?:\.|%2[eE]|%252[eE]|%%32%65)(?:/|%2[fF]|%252[fF]|%%32%66))[^"]*" \d{3}
            ^.* \[error\] \d+#\d+: \*\d+ .* client: <HOST>, .* request: "(?:GET|POST|HEAD|PUT|PATCH|DELETE) [^"]*(?:\.\./\.\./|\x252E\x252E\x252F|\.(?:\.|%2[eE]|%252[eE]|%%32%65)(?:/|%2[fF]|%252[fF]|%%32%66))
EOF
    fi
