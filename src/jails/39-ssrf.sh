...
# [PYTHON 3.11+ FIX] Moved inline global flag (?i) to the absolute beginning of the string to prevent compilation errors.
failregex = (?i)^<HOST> \S+ \S+ (?:\[[^\]]*\]\s+)?"(?:GET|POST|HEAD|PUT|DELETE|PATCH|OPTIONS) [^"]*?(?:169\.254\.169\.254|2852039166|0xa9fea9fe|0251\.0376\.0251\.0376|0xa9\.0xfe\.0xa9\.0xfe|\x2531\x2536\x2539\x252e|(?:/|\x252f|\x25252f)metadata(?:/|\x252f|\x25252f)(?:instance|identity|v1)|latest(?:/|\x252f|\x25252f)meta-data|metadata\.google\.internal|(?:/|\x252f|\x25252f)v1(?:/|\x252f|\x25252f)user-data|100\.100\.100\.200|192\.0\.0\.192).* HTTP/.*" \d{3} .*$
ignoreregex = 
EOF
    fi
