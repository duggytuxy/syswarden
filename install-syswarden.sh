...
EOF
        ...
        if [[ "${USE_WIREGUARD:-n}" == "y" ]]; then
            ...
            # --- STRICT ZERO TRUST HIERARCHY (v1.44) - DEBIAN PARITY) ---
...
uninstall_syswarden() {
    ...
    # --- DEVSECOPS FIX: SCORCHED EARTH FAIL2BAN PURGE ---
    log "INFO" "Executing Scorched Earth purge on Fail2ban memory and logs..."
    # 1. Brutal kill to prevent SQLite locking
...
if [[ "$MODE" != "update" ]]; then
    clear
    echo -e "${GREEN}#############################################################"
    echo -e "#     SysWarden Tool Installer (Universal v1.44)     #"
    echo -e "#############################################################${NC}"
fi
...
# Dependencies MUST be checked during updates to ensure new modules (like pyinotify) are installed.
install_dependencies
...
if [[ "$MODE" == "update" ]]; then
    log "INFO" "Update Mode: Re-applying Fail2ban optimizations (PyInotify)..."
    configure_fail2ban
fi
