...
### FIXED 🐛
- **nftables qualification workspace:** Create a real, owner-controlled 0700 temporary directory below the isolated qualification root, prove it writable before use, pass it to only the nftables laboratory as `TMPDIR` and `GOTMPDIR`, and remove it through the bounded always-run cleanup.
- **Bubblewrap temporary isolation:** Reset `TMPDIR` only for the re-executed firewall test inside Bubblewrap to its private `/tmp` tmpfs, so an inherited runner path cannot become read-only behind the sandbox root bind.
- **FreeBSD mode preflight:** Verify the `/tmp` sticky bit with FreeBSD's separate `%Mp` and `%Lp` mode fields in both the VM laboratory and signed-updater probe instead of expecting `%Lp` alone to contain the high bit.
- **FreeBSD package catalogue:** Force a bounded, fail-closed `pkg update -f` before installing standalone TXZ prerequisites, retain the diagnostic only in private laboratory output, and use distinct nonzero statuses for refresh and dependency failures.
- **FreeBSD laboratory stdin and timeouts:** Redirect file descriptor 0 from `/dev/null` for both `script` TUI probes and all eleven noninteractive `pkg` update, install, add, delete and cleanup invocations. Together, the eleven package commands and two TUI probes create exactly 13 explicit `</dev/null` boundaries. All six lifecycle `pkg add -f` and two evidence-bearing `pkg delete -fy` invocations also use bounded FreeBSD `timeout -f`, so successfully detached service children are not reaped. This prevents FreeBSD `script`, which previously inherited the remote `/bin/sh -s` control stream, from consuming unread shell source and returning status 2.
- **FreeBSD signal cleanup:** Preserve the original status through the EXIT trap, map HUP, INT and TERM to statuses 129, 130 and 143, and route every exit through one final cleanup invocation.
- **FreeBSD cron access proof:** Write `root` and `nobody` on separate lines in `/var/cron/allow`, validate the access files as byte-exact nonsymlink `root:wheel` 0600 state, and require a transactional byte-exact root crontab probe and restoration before candidate upgrade plus checked byte-exact seed/readback before package removal. Once the probe runs, restore the original present or absent state even after a primary failure, and fail closed on any primary, restoration or restoration-readback error.
- **FreeBSD PF grammar:** Use the FreeBSD 14.4 singular `fragment` keyword in both the generator and frozen v4.02.8 fixture, while the regression contract rejects the invalid plural `fragments` form.
- **FreeBSD PF recovery snapshot:** Publish schema v2 before any PF module load or policy mutation, record `initial_kernel_state=module_absent` when that is the captured host state, and include the state table read through `pfctl -ss`. Persist `mutation_started=true` immediately before applying the validated policy. Restoration unloads PF only when the snapshot recorded an initially absent module; the qualification laboratory exercises that absent-module install and removal path and returns the guest to module and control-device absence.
- **FreeBSD SSH idempotence:** After preserving the clean temporary and rollback path guards, when normalized `sshd_config` content is byte-identical, perform zero backup, temporary-file write, candidate validation, rename, directory sync or `sshd` restart. Changed content retains the validated compare-and-swap transaction and rollback behavior.
- **FreeBSD package ownership:** Normalize every TXZ member to numeric UID and GID 0 with `root:wheel` names, remove PAX `uid`, `gid`, `uname` and `gname` overrides before writing the archive, and make package verification reject any numeric or named owner or group drift.
- **FreeBSD result attribution:** Preserve a redacted primary laboratory error when transport cleanup also fails, and report the cleanup failure separately. A valid primary result advances only after cleanup succeeds within the three bounded attempts; exhaustion returns status 92 and does not publish that successful result.
- **Qualification diagnostics:** Enumerate every nonzero laboratory status and expose only an allowlisted failure summary, while keeping raw reports, transport material and secrets outside public artifacts.

### SECURITY 🔒
- **Layered temporary boundaries:** Keep the parent nftables harness in its qualification-owned directory and the Bubblewrap child in its own tmpfs; neither layer falls back to the runner-controlled temporary path that caused the v4.02.14 harness failure.
- **Adversarial cleanup proof:** Reject symlink, ownership, mode and path-escape mutations before deleting nftables temporary state, and keep the release closed if cleanup cannot be proven.
- **FreeBSD fail-closed proof:** Reject an incorrect sticky-bit state, a stale or unreadable package catalogue and any prerequisite-install failure before the historical or candidate package lifecycle begins.
- **FreeBSD control-stream isolation:** Enforce exactly 13 explicit stdin boundaries across eleven package commands and two pseudo-terminal probes, while one status-preserving cleanup path handles normal exits and explicitly mapped signals. The six lifecycle package additions and two evidence-bearing removals retain foreground-safe bounded execution.
- **FreeBSD lifecycle preconditions:** Reject combined cron-access usernames, a missing root entry, ownership or mode drift, a failed crontab write and any byte-mismatched readback before the package lifecycle can advance.
- **FreeBSD policy and transport stability:** Keep the PF generator and frozen fixture on the parser-accepted singular fragment grammar, and avoid disrupting SSH transport for a true byte-identical configuration while preserving the full transaction for changed content.
- **FreeBSD PF state fidelity:** Bind recovery to schema v2, the captured initial module state, live rules, tables, anchors and `pfctl -ss` states. Cross the durable mutation boundary only immediately before apply, never unload an initially present PF module during restore, and prove the initially absent path returns the guest to absence.
- **FreeBSD archive identity:** Prevent build-host ownership from reaching privileged binaries, rc.d scripts, signatures or package manifests by enforcing one exact root-owned identity across every archive member.
- **Release evidence minimization:** Report failure coordinates and bounded diagnostics without uploading raw laboratory documents, private `output/` material or secret-bearing transport files.

### RELEASE STATUS 📦
- v4.02.14 was merged as a source candidate but was never tagged or published. Protected qualification run `31993252784` completed the native amd64 and ARM64 package lifecycle shards, but the FreeBSD harness stopped before sealed package inputs because its sticky-bit check misread the platform mode fields, and the nftables harness stopped before product evaluation because the Bubblewrap child inherited a runner temporary path through a read-only root bind. FreeBSD and nftables product behavior was therefore not evaluated by that run.
- No sealed v4.02.14 qualification artifact, release signature, tag, public Release or Release asset was created. The passed package evidence is bound to the failed v4.02.14 candidate and cannot authorize v4.02.15.
- The final disposable FreeBSD VM evidence was generated at `2026-08-17T11:52:09.684249+00:00`. Its report SHA-256 is `099015d7d7433235b50a2d3cff76782553536af2fe2357dfb5ebf5f999e085`; it binds candidate package `syswarden-4.02.15.txz` at SHA-256 `e3ff75d5961092ca1ab0ec752e8dd70d4121f9bf2cbb26957dc339cdd276bf65`.
- The final result is `harness_status=pass`, `product_status=fail` and `release_ready=false`. Of 75 checks, 68 passed and seven unexpected checks have status `blocker`.
- The seven exact unexpected check IDs are `SW-PKG-FBSD-CANDIDATE-UPGRADE-POSTINSTALL-001`, `SW-PKG-FBSD-CANDIDATE-REINSTALL-POSTINSTALL-001`, `SW-PKG-FBSD-CANDIDATE-RESTART-IDEMPOTENCE-POSTINSTALL-001`, `SW-PKG-FBSD-RCD-ENABLE-001`, `SW-PKG-FBSD-UPGRADE-RCD-001`, `SW-PKG-FBSD-PF-MODULE-ABSENT-INSTALL-001` and `SW-PKG-FBSD-RSYSLOG-001`.
- In all three candidate post-install checks, the complete modular configuration inventory is present, while `diagnostics_clean=0` and the expected post-install marker is absent. The general rc.d check reports both enable values empty; the immediate post-upgrade rc.d check reports both enable values empty and both service status codes equal to 1.
- The module-absent PF install check reports schema version 2, `initial_kernel_state=module_absent`, exact-live provenance, a safe snapshot, successful package return code 0, a present module and ready control device, but `diagnostics_clean=0`, `mutation_started=false`, policy status `Disabled`, rule count 0 and an absent post-install marker. The rsyslog check reports validation return code 0, an empty enable value, service status return code 1 and `base_syslogd_inactive=0`.
- v4.02.15 therefore remains NO-GO. Publication is stopped. No v4.02.15 tag, public Release, Release signature or public asset is authorized or created. The last public Release remains v4.02.8.
- Work resumes on 20 August 2026 at 13:00 CEST with the recorded plan to remove FreeBSD support completely. Until that work is completed and independently qualified, no FreeBSD release claim is authorized.
- The planned but unpublished Release inventory was seven native packages plus `SHA256SUMS.txt`, `RELEASE_SHA256SUMS.txt`, `syswarden-release.tar.gz`, `syswarden-sbom.spdx.json`, `plumber-report.zip`, `syswarden-update-manifest-v1.json` and `syswarden-update-manifest-v1.json.sig`, exactly 14 assets. No PDF and no private `output/` file belongs to a public Release.

### UPGRADE NOTES 📦
- v4.02.15 is an unpublished failed candidate. Do not install it as a public Release and do not use `syswarden update` to target it.
- v4.02.8 remains the last public Release. No package command, candidate checksum or local qualification file converts v4.02.15 into an authorized Release.
- FreeBSD operators must not treat the candidate TXZ or its local VM evidence as production authorization. The complete FreeBSD withdrawal plan starts when work resumes on 20 August 2026 at 13:00 CEST.
...
## ADDED ➕
- **Syswarden-TUI**: Added a `SEVERITY` column in the "TOP ATTACKERS (OSINT HISTORY)" table, which dynamically calculates and displays the aggressiveness percentage (`HitCount / TotalBans`) of each attacker.

## UPDATED ♻️
- **Telemetry / TUI**: Overhauled the `Noise/Signal` calculation for the Zero-Trust (ALLOW) mode. The TUI now detects `SYSWARDEN_GEO_ALLOWED` or `SYSWARDEN_ASN_ALLOWED` and correctly overrides the Noise metric to `>99.99% (Zero-Trust)`, avoiding miscalculations and heavy CPU overhead.
